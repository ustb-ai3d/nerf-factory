...
from src.data.litdata import LitDataNeuroFluid1Frame
...
def run(
    ...
    # load dataset here
    data_module_all = select_dataset(
    ...
    frame_num = data_module_all.frame_num
    for frame_iter in range(frame_num):

        model_dir = os.path.join(logdir, "model_{}".format(frame_iter))
        ...
        if run_train:
            txt_path = os.path.join(model_dir, "config.gin")
        ...
        print("\nNow loading model_{}".format(frame_iter))

        model = select_model(model_name=model_name)
        model.logdir = model_dir

        data_module = LitDataNeuroFluid1Frame(frame_i=frame_iter,
                                              data_module_all=data_module_all,
                                              datadir=datadir)
        ...
        if run_train:
            ...
            if os.path.exists(best_ckpt):
                os.remove(best_ckpt)
            version0 = os.path.join(model_dir, exp_name, "version_0")
        ...
        if run_eval:
            ckpt_path = (
                f"{model_dir}/best.ckpt"
                if model_name != "mipnerf360"
                else f"{model_dir}/last.ckpt"
            )
